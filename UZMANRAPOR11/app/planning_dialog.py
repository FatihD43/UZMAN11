...
NEVER = set()
HAM_ALLOWED = set(range(1301, 1913))   # ISKO11: tüm aralık
DENIM_ALLOWED_RANGE = (1301, 1912)     # ISKO11: 1301–1912
...
def _tarak_key_generic(val) -> str:
    """Dinamik/Running ile aynı normalize: a/b/c. Leading zero'ları düşürür, ondalığı korur."""
    if val is None or (isinstance(val, float) and pd.isna(val)):
        return ""
    ...
    for n in nums[:3]:
        ...
        # "054.0" -> "54", "04" -> "4", "052.5" -> "52.5"
        if re.fullmatch(r"\d+\.\d+", n):
            # ondalıklıysa: integer kısmındaki leading zero'ları düşür
            a, b = n.split(".", 1)
            a = str(int(a)) if a.isdigit() else a
            # "0" gibi durumlar için b'yi koru
            n = f"{a}.{b}".rstrip("0").rstrip(".")
        else:
            # tam sayı gibi ise: leading zero düşür
            if n.isdigit():
                n = str(int(n))

        # "54.0" zaten yukarıda temizleniyor ama garanti:
...
class PlanningDialog(QDialog):
    ...
    def __init__(
        ...
        self.df_looms = self._build_all_looms_with_overlay(df_looms)
    ...
    def _build_all_looms_with_overlay(self, df_running: pd.DataFrame) -> pd.DataFrame:
        """ISKO11: Running sadece çalışan tezgahları içerir. Planlama ekranında 'Boş Tezgahlar'
        gösterebilmek için tüm tezgahları (1301–1912) baz alır.
        Tarak Grubu öncelik: Running['Tarak Grubu'] doluysa onu kullanır; boş kalanlar için
        LoomLastState (Makine->TarakGrubu) ile tamamlar.
        Üretilen df'de:
          - Tezgah (int)
          - Tarak Grubu (str)
          - _TarakKey (normalize)
          - _IsRunning (bool)
          - _OpenTezgahFlag (bool)  -> Running'de olmayanlar True
          - _KalanMetreNorm (float) -> Running yoksa 999999
        """
        # 1) Base: tüm tezgahlar
        df_all = pd.DataFrame({"Tezgah": list(range(1301, 1913))})

        # 2) LoomLastState fallback map
        tarak_map: dict[int, str] = {}
        try:
            from app.sql_api_client import run_sql
            from app.db_name import DB_NAME
            sql = f"""
                SELECT Makine, TarakGrubu
                FROM [{DB_NAME}].dbo.LoomLastState
                WHERE TarakGrubu IS NOT NULL
            """
            df_ls = run_sql(sql)
            if df_ls is not None and not df_ls.empty:
                df_ls = df_ls.copy()
                df_ls["Makine"] = df_ls["Makine"].astype(int)
                df_ls["TarakGrubu"] = df_ls["TarakGrubu"].astype(str)
                tarak_map = dict(zip(df_ls["Makine"], df_ls["TarakGrubu"]))
        except Exception:
            tarak_map = {}

        df_all["Tarak Grubu"] = df_all["Tezgah"].map(tarak_map).fillna("").astype(str)

        # 3) Running overlay
        df_run = df_running.copy() if df_running is not None else pd.DataFrame()
        loom_col = _pick_col(df_run, ["Tezgah", "Tezgah No", "Tezgah Numarası", "Makine", "Loom"])
        if df_run.empty or not loom_col:
            df_all["_IsRunning"] = False
            df_all["_OpenTezgahFlag"] = True
            df_all["_KalanMetreNorm"] = 999999.0
            df_all["_TarakKey"] = df_all["Tarak Grubu"].astype(str).apply(_tarak_key_generic)
            return df_all
        ...
        df_run["_TezgahInt"] = df_run[loom_col].apply(_to_int)
        df_run = df_run[df_run["_TezgahInt"].notna()].copy()
        df_run["_TezgahInt"] = df_run["_TezgahInt"].astype(int)

        tg_col = _pick_col(df_run, ["Tarak Grubu", "TarakGrubu", "Tarak"])
        if tg_col:
            df_run["_TG_run"] = df_run[tg_col].astype(str).fillna("").str.strip()
        else:
            df_run["_TG_run"] = ""

        km_col = _pick_col(df_run, ["KalanMetre", "Kalan Metre", "Kalan", "Remaining", "MetreKalan", "OrderRemaining"])
        if km_col:
            df_run["_KalanMetreRaw"] = df_run[km_col]
        else:
            df_run["_KalanMetreRaw"] = None

        df_ov = df_run[["_TezgahInt", "_TG_run", "_KalanMetreRaw"]].drop_duplicates(subset=["_TezgahInt"], keep="first")
        df_all = df_all.merge(df_ov, left_on="Tezgah", right_on="_TezgahInt", how="left")

        df_all["_IsRunning"] = df_all["_TezgahInt"].notna()

        # Tarak Grubu override: running’de doluysa onu kullan
        mask_tg = df_all["_TG_run"].astype(str).str.strip().ne("")
        df_all.loc[mask_tg, "Tarak Grubu"] = df_all.loc[mask_tg, "_TG_run"]

        # Open flag: running yoksa boş tezgah => açık
        df_all["_OpenTezgahFlag"] = ~df_all["_IsRunning"]
        ...
        df_all["_KalanMetreNorm"] = df_all["_KalanMetreRaw"].apply(_km_norm)

        df_all = df_all.drop(columns=["_TezgahInt", "_TG_run"], errors="ignore")

        # Planlama filtresi için key
        df_all["_TarakKey"] = df_all["Tarak Grubu"].astype(str).apply(_tarak_key_generic)
    ...
    def _current_key(self) -> str:
        ...
        if not rows.empty:
            if "_TarakKey" in rows.columns and pd.notna(rows["_TarakKey"]).any():
                return _tarak_key_generic(rows["_TarakKey"].iloc[0])  # <-- normalize et
    ...
    def _load_looms_for_key_and_category(self, key: str, category: str):
        ...
        if self.df_looms is None:
            return
        df = self.df_looms.copy()
        print("\n--- PLANNING DEBUG ---")
        print("category:", category, " key:", key)
        print("df_looms rows:", 0 if self.df_looms is None else len(self.df_looms))
        print("df_jobs assigned count:",
              int(self.df_jobs.get("Tezgah Numarası", pd.Series([], dtype=object)).astype(str).str.strip().ne(
                  "").sum()))
        ...
        print("after TarakKey filter rows:", len(df))

        if len(df) == 0:
            # TarakKey uyuşmazlığını görmek için örnek değerleri yaz
            if self.df_looms is not None and ("_TarakKey" in self.df_looms.columns):
                print("sample df_looms _TarakKey:", self.df_looms["_TarakKey"].astype(str).head(10).tolist())
            else:
                tg_col0 = _pick_col(self.df_looms,
                                    ["Tarak Grubu", "Tarak", "TarakGrubu"]) if self.df_looms is not None else None
                print("tg_col:", tg_col0)
            return

        # Boş tezgah mantığı: Running’de olmayanlar (çalışmayanlar) her zaman "boş/açık" kabul edilir.
        if "_IsRunning" in df.columns:
            try:
                df.loc[df["_IsRunning"] == False, "_OpenTezgahFlag"] = True
            except Exception:
                pass
            try:
                nr = int((df["_IsRunning"] == False).sum())
                print("non-running rows after TarakKey:", nr)
            except Exception:
                pass
        ...
        if "_OpenTezgahFlag" not in df.columns:
            def _detect_94_row(row):
                for c in row.index:
                    v = row.get(c, "")
                    # Sayısalsa 94 kontrolü
                    try:
                        if pd.notna(v) and float(v) == 94.0:
                            return True
                    except Exception:
                        pass

                    u = str(v).strip().upper()
                    if "SİPARİŞ YOK" in u or "SIPARIS YOK" in u:
                        return True

                    # Metinse "94" / "94.0" / " 94" gibi
                    if re.fullmatch(r"94(?:\.0+)?", u):
                        return True

                return False
            df["_OpenTezgahFlag"] = df.apply(_detect_94_row, axis=1)
            print("open flag true count:", int(df["_OpenTezgahFlag"].sum()))
        ...
        assigned_raw = self.df_jobs.get("Tezgah Numarası", pd.Series([], dtype=object)).astype(str)
        assigned_looms = set()
        for v in assigned_raw.tolist():
            m = re.search(r"(\d+)", str(v))
            if m:
                assigned_looms.add(m.group(1))
        ...
        print("after allowed_row filter rows:", len(df))
        ...
        # Boş tezgahlar: çalışmayanlar (running’de olmayanlar) + (opsiyonel) açık flag
        if "_IsRunning" in df.columns:
            free_df = df[df["_IsRunning"] == False].copy()
        else:
            free_df = df[df["_OpenTezgahFlag"] == True].copy()
        soon_df = df[(df["_OpenTezgahFlag"] != True) & (df["_KalanMetreNorm"] <= thr)].copy()
        print("free_df rows:", len(free_df))
        print("soon_df rows:", len(soon_df))
