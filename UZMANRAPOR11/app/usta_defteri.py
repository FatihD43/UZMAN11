...
class UstaDefteriWidget(QWidget):
    ...
    def set_machine_list(self, looms: List[str]):
        full = [str(x) for x in range(1301, 1912)]
    ...
    def _fetch_lookup_rows(self, list_name: str) -> List[Dict[str, object]]:
        sql = f"""
        SELECT Id, Value
        FROM [{DB_NAME}].[dbo].[AppLookupValues]
    ...
    def _ensure_lookup_value(self, list_name: str, value: str, created_by: str = "AUTO") -> None:
        ...
        with self._conn() as c:
            cur = c.cursor()
            cur.execute(
                f"SELECT 1 FROM [{DB_NAME}].[dbo].[AppLookupValues] WHERE ListName = ? AND Value = ?;",
                (list_name, value),
            )
            ...
            cur.execute(
                f"INSERT INTO [{DB_NAME}].[dbo].[AppLookupValues] (ListName, Value, IsActive, SortOrder, CreatedBy) "
                "VALUES (?, ?, 1, 0, ?);",
    ...
    def _update_lookup_value(self, row_id: int, new_value: str, updated_by: str = "UI") -> None:
        ...
        if not new_value:
            return
        sql = f"""
        UPDATE [{DB_NAME}].[dbo].[AppLookupValues]
    ...
    def _build_form_box(self) -> QGroupBox:
        ...
        self.cmb_tezgah.addItem("")
        self.cmb_tezgah.addItems([str(x) for x in range(1301, 1912)])
        ...
        # --- FA / SU alanlarını yan yana düzenle ---
        # r+5 : FA satırı
        grid.addWidget(QLabel("LEVENT NO (FA)"), r + 6, 0)
        self.ed_levent = QLineEdit()
        grid.addWidget(self.ed_levent, r + 6, 1)

        grid.addWidget(QLabel("ETİKET NO (FA)"), r + 6, 2)
        self.ed_etiket = QLineEdit()
        grid.addWidget(self.ed_etiket, r + 6, 3)

        # r+6 : SU satırı
        grid.addWidget(QLabel("LEVENT NO (SU)"), r + 7, 0)
        self.ed_levent_su = QLineEdit()
        grid.addWidget(self.ed_levent_su, r + 7, 1)

        grid.addWidget(QLabel("ETİKET NO (SU)"), r + 7, 2)
        self.ed_etiket_su = QLineEdit()
        grid.addWidget(self.ed_etiket_su, r + 7, 3)

        grid.addWidget(QLabel("AÇIKLAMA"), r + 8, 0)
        self.ed_aciklama = QLineEdit()
        grid.addWidget(self.ed_aciklama, r + 8, 1, 1, 3)  # 1..3 arası span yeterli
    ...
    def _insert_row(self, rec: Dict):
        ...
        sql = f"INSERT INTO [{DB_NAME}].[dbo].[UstaDefteri] ({','.join(cols)}) VALUES ({placeholders})"
    ...
    def _delete_by_rowid(self, rowid: int):
        with self._conn() as c:
            cur = c.cursor()
            cur.execute(f"DELETE FROM [{DB_NAME}].[dbo].[UstaDefteri] WHERE Id = ?", (rowid,))
            c.commit()
    ...
    def _select(self, start: Optional[str] = None, end: Optional[str] = None,
        ...
        SELECT
            ...
            LeventNo AS [Levent FA],
            LeventSU AS [Levent SU],
            EtiketNo AS [Etiket FA],
            EtiketSU AS [Etiket SU],
    ...
    def _clear_form(self):
        ...
        for w in [
            ...
            w.clear()
        self.ed_levent_su.clear()
        self.ed_etiket_su.clear()
    ...
    def _on_levent_bul(self):
        ...
        # --- SU alanlarını doldur (kolon adları toleranslı) ---
        def _get_first(row_obj, keys):
            for k in keys:
                if k in row_obj:
                    v = row_obj.get(k, "")
        ...
        levent_su_val = _get_first(row, ["LeventSu", "Levent SU", "Levent No SU", "LeventNoSU"])
        etiket_su_val = _get_first(row, ["EtiketSu", "Etiket SU", "Levent Etiket SU", "EtiketNoSU"])

        self.ed_levent_su.setText(_strip_trailing_dot_zero(levent_su_val))
        self.ed_etiket_su.setText(_strip_trailing_dot_zero(etiket_su_val))
    ...
    def _on_save(self):
        try:
            ...
            levent_su = self.ed_levent_su.text().strip() if hasattr(self, "ed_levent_su") else ""
            etiket_su = _strip_trailing_dot_zero(self.ed_etiket_su.text().strip()) if hasattr(self, "ed_etiket_su") else ""
    ...
    def _etiket_exists(self, etiket: str) -> bool:
        ...
        with self._conn() as c:
            cur = c.cursor()
            cur.execute(f"SELECT 1 FROM [{DB_NAME}].[dbo].[UstaDefteri] WHERE EtiketNo = ?;", (etiket,))
