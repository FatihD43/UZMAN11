...
def _fill_tarak_grubu_from_sql(df_running: pd.DataFrame) -> pd.DataFrame:
    if df_running is None or df_running.empty:
        return df_running

    # Running'de Kök Tip kolonu
    tip_col = None
    for c in ["Kök Tip Kodu", "KökTip", "Malzeme"]:
        if c in df_running.columns:
            tip_col = c
            break
    if not tip_col:
        return df_running
    ...
    # TarakGrubu tablosunu çek
    from app.db_name import DB_NAME
    with get_sql_connection() as conn:
        cur = conn.cursor()
        cur.execute(f"""
            SELECT
                UPPER(LTRIM(RTRIM(Malzeme))) AS KokTip,
                LTRIM(RTRIM(Karakteristik))  AS TarakGrubu
            FROM [{DB_NAME}].[dbo].[TarakGrubu]
        """)
        rows = cur.fetchall()
    ...
    # Map oluştur
    tarak_map = {r[0]: r[1] for r in rows if r[0]}
    ...
    mask = df["Tarak Grubu"].astype(str).str.strip().eq("")

    df.loc[mask, "Tarak Grubu"] = (
        df.loc[mask, tip_col]
        .astype(str)
        .str.strip()
        .str.upper()
...
class LoomListEditor(QDialog):
    ...
    def __init__(self, title: str, settings_key: str, parent=None):
        ...
        self.settings = QSettings("UZMANRAPOR11", "ClientApp")
...
class MainWindow(QMainWindow):
    ...
    def load_running(self):
        ...
        try:
            ...
            df = _fill_tarak_grubu_from_sql(df)  # ✅ burada
            df = normalize_df_running(df)  # ✅ sonra normalize
            ...
            # Running gelenleri LoomLastState’e yaz (Running’de olmayanlara dokunmaz)
            storage.upsert_loom_last_state_from_running(self.df_running, source="running")
