...
def _norm_tarak_generic(s: str) -> str:
    ...
    return "/".join(out)
def _ensure_open_and_kalan_fields(df: pd.DataFrame) -> pd.DataFrame:
    """
    df_running içinde planner’ın ihtiyaç duyduğu alanları garanti eder:
      - _OpenTezgahFlag  (AÇIK mı?)
      - _KalanMetreNorm  (sayısal kalan metre)
    """
    ...
    # 2) _OpenTezgahFlag (AÇIK tespiti)
    # Önce "açık" sayılabilecek metinleri arayacağız.
    # df_running’de hangi kolon varsa onu kullanacağız.
    col_durum = _col(r, ["Durum", "Durum Tanım", "Durum Açıklaması", "Durumu"])
    col_tip   = _col(r, ["Tip", "KökTip", "Kök Tip Kodu", "Mamul Tipi"])

    def _row_is_open(x) -> bool:
        # a) Eğer bir kolonda "SİPARİŞ YOK" / "94" gibi açık sinyali varsa
        # (senin planning_dialog’daki _detect_94_row mantığıyla uyumlu)
        for c in x.index:
            v = x.get(c, "")
            try:
        ...
        # b) Durum kolonu varsa ondan da yakala
        if col_durum and col_durum in x.index:
            u = str(x.get(col_durum, "")).upper()
            if ("AÇIK" in u) or ("ACIK" in u) or ("BOŞ" in u) or ("BOS" in u):
                return True

        # c) Tip boşsa da açık kabul edebiliriz (bazı running formatlarında açık tezgah tip boş gelir)
        if col_tip and col_tip in x.index:
            t = str(x.get(col_tip, "")).strip()
            if t == "" or t.lower() in ("nan", "none"):
                return True

        return False

    r["_OpenTezgahFlag"] = r.apply(_row_is_open, axis=1)
    return r
...
class TeamPlanningFlowTab(QWidget):
    ...
    def refresh_sources(self):
        ...
        # >>> NOKTA ATIŞI: running alanlarını burada garanti et
        if self.df_run is not None and not self.df_run.empty:
            self.df_run = _ensure_open_and_kalan_fields(self.df_run)
        # <<<
