...
def fetch_tip_buzulme_model(tip_kodlari: list[str]) -> pd.DataFrame:
    ...
    return res
# ============================================================
#  LOOM LAST STATE (SQL)  -  ISKO11 Kuşbakışı besleme
#  Kolonlar: Makine, DBirim, TarakGrubu, UpdatedAt, Source
# ============================================================

def load_loom_last_state_map() -> dict[str, dict[str, str]]:
    """
    { "1405": {"DBirim":"R54632", "TarakGrubu":"052.5/04/182"} , ... }
    """
    sql = f"""
    SELECT
        CAST(Makine AS nvarchar(20)) AS Makine,
        CAST(DBirim AS nvarchar(50)) AS DBirim,
        CAST(TarakGrubu AS nvarchar(50)) AS TarakGrubu
    ...
    for _, r in df.iterrows():
        mk = str(r.get("Makine", "") or "").strip()
        if not mk:
            continue
        out[mk] = {
            "DBirim": (str(r.get("DBirim", "") or "").strip()),
            "TarakGrubu": (str(r.get("TarakGrubu", "") or "").strip()),
        }
    return out


def upsert_loom_last_state_from_running(df_running: pd.DataFrame | None, source: str = "running") -> None:
    """
    Running’den gelen tezgahları LoomLastState’e yazar:
    - Running’de varsa: UPDATE (ve yoksa INSERT)
    - Running’de yoksa: dokunmaz (SQL’deki eski hali kalır)
    Beklenen Running kolonları: Makine, Kök Tip Kodu, Tarak Grubu
    """
    ...
    # Running kolon isimleri sabit: Makine, Kök Tip Kodu, Tarak Grubu
    # SQL kolonları: Makine, DBirim, TarakGrubu
    rows: list[tuple[str, str, str]] = []
    for _, r in df_running.iterrows():
        mk = _clean(r.get("Makine"))
        if not mk:
            continue
        dbirim = _clean(r.get("Kök Tip Kodu"))
        tg = _clean(r.get("Tarak Grubu"))
        if (not dbirim) and (not tg):
            continue
        rows.append((mk, dbirim, tg))
    ...
    try:
        with _sql_conn() as c:
            ...
            for (mk, dbirim, tg) in rows:
                # UPDATE
                cur.execute(
                    f"""
                    UPDATE [{DB_NAME}].[dbo].[LoomLastState]
                    SET DBirim = ?, TarakGrubu = ?, UpdatedAt = SYSUTCDATETIME(), Source = ?
                    WHERE Makine = ?;
                    """,
                    (dbirim, tg, source, mk),
                )

                # INSERT if not exists  (API IF/EXISTS engelliyor -> SELECT ile kontrol)
                cur.execute(
                    f"SELECT TOP 1 1 FROM [{DB_NAME}].[dbo].[LoomLastState] WHERE Makine = ?;",
                    (mk,),
                )
                ...
                if not exists:
                    cur.execute(
                        f"""
                        INSERT INTO [{DB_NAME}].[dbo].[LoomLastState]
                        (Makine, DBirim, TarakGrubu, UpdatedAt, Source)
                        VALUES (?, ?, ?, SYSUTCDATETIME(), ?);
                        """,
                        (mk, dbirim, tg, source),
                    )
        ...
        print(f"[LoomLastState] upsert hatası: {e!r}")
