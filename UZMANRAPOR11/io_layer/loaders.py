...
from app.storage import load_loom_cut_map, load_type_selvedge_map, save_type_selvedge_map



VISIBLE_COLUMNS = [
    "Tezgah Numarası", "Kök Tip Kodu",
    "Levent No", "Levent Etiket FA", "EtiketSu", "LeventSu", "Tarak Grubu", "Zemin Örgü", "Üretim Sipariş No", "Haşıl İş Emri",
...
def load_dinamik_any(path: str|Path) -> pd.DataFrame:
    ...
    df = df.copy()
    # --- ISKO11: Excel başlıklarını normalize et ---
    rename_map = {
        "Levent Etiket SU": "EtiketSu",
        "Levent No SU": "LeventSu",
    }
    df = df.rename(columns={k: v for k, v in rename_map.items() if k in df.columns})

    # Kolon yoksa aç (fallback)
    if "EtiketSu" not in df.columns:
        df["EtiketSu"] = ""
    if "LeventSu" not in df.columns:
        df["LeventSu"] = ""
...
def load_running_orders(path: str|Path) -> pd.DataFrame:
    """ISKO11 Running Orders mapping:
    - Makine -> Tezgah No
    - D-Sipariş -> EtiketNoFA
    - D-Birim -> Kök Tip Kodu (noktaya kadar, örn ABC.123 -> ABC)
    - Rem. O-Length -> Kalan Mt
    - ÜD-Kodu -> Duruş
    - Çift fon kuralı: aynı tezgah için Ü-Hattı=2 varsa sadece o; yoksa Ü-Hattı=1
    """
    df = pd.read_excel(path, sheet_name=0, engine="openpyxl")
    df = df.copy()
    ...
    c_makine = pick(["Makine", "MAKINE", "Tezgah", "Tezgah No", "Tezgah_No"])
    if c_makine:
        df["Tezgah No"] = df[c_makine]
    else:
        df["Tezgah No"] = ""

    c_sip = pick(["D-Sipariş", "D-Siparis", "D-Sipariş ", "D-Siparis "])
    if c_sip:
        df["EtiketNoFA"] = df[c_sip]
    elif "EtiketNoFA" not in df.columns:
        df["EtiketNoFA"] = ""

    c_birim = pick(["D-Birim", "D-Birim ", "D-Birim/Tip", "D-Birim Kodu"])
    if c_birim:
        s = df[c_birim].astype(str)
        df["Kök Tip Kodu"] = s.str.split(".", n=1).str[0].str.strip()
    elif "Kök Tip Kodu" not in df.columns:
        df["Kök Tip Kodu"] = ""

    c_kalan = pick(["Rem. O-Length", "Rem O-Length", "Rem. O Length", "Kalan Mt", "Kalan", "Kalan Metre"])
    if c_kalan:
        df["_KalanMetre"] = pd.to_numeric(df[c_kalan], errors="coerce").fillna(0.0)
    else:
        df["_KalanMetre"] = 0.0

    c_ud = pick(["ÜD-Kodu", "UD-Kodu", "ÜD Kodu", "UD Kodu", "Duruş", "Durus"])
    if c_ud:
        df["Duruş"] = df[c_ud]
    elif "Duruş" not in df.columns:
        df["Duruş"] = ""

    # Durus No: varsa sayısal yakala
    durus_no_col = None
    for c in df.columns:
        cn = _norm_upper(c)
        if cn.startswith("DURUS") or cn.startswith("DURU"):
            ...
            break
    if durus_no_col:
        df["Durus No"] = pd.to_numeric(df[durus_no_col], errors="coerce").fillna(0).astype(int)
    else:
        df["Durus No"] = 0

    # Çift fon seçimi
    c_hat = pick(["Ü-Hattı", "U-Hattı", "Ü Hattı", "U Hattı", "U_Hatti", "Ü_Hattı"])
    if c_hat:
        df["_U_Hatti"] = pd.to_numeric(df[c_hat], errors="coerce").fillna(0).astype(int)
        # tezgah bazında 2 varsa 2 seç
        def _pick_group(g: pd.DataFrame) -> pd.DataFrame:
            if (g["_U_Hatti"] == 2).any():
                return g[g["_U_Hatti"] == 2].head(1)
            return g[g["_U_Hatti"] == 1].head(1) if (g["_U_Hatti"] == 1).any() else g.head(1)
        df = df.groupby("Tezgah No", dropna=False, as_index=False, sort=False).apply(_pick_group).reset_index(drop=True)
    else:
        # zaten tek satır varsay
        pass

    # ISKO11: Running'de Tarak Grubu yok -> boş bırak, GUI normalize içinde SQL'den doldurur
    if "Tarak Grubu" not in df.columns:
        df["Tarak Grubu"] = ""
    df["_TarakKey"] = df["Tarak Grubu"].apply(_tarak_key)

    # Açık tezgah flag
    df["_OpenTezgahFlag"] = False
...
def enrich_running_with_selvedge(df_run: pd.DataFrame, df_dinamik: pd.DataFrame) -> pd.DataFrame:
    ...
    return df_out
def enrich_running_with_tarakgrubu(df_run: pd.DataFrame, tarak_map: dict[str, str]) -> pd.DataFrame:
    """
    tarak_map: {KOKTIP: TARAK_GRUBU}
    """
    if df_run is None or df_run.empty or not tarak_map:
        return df_run

    # running'de kök tip kolonu hangisi?
    col_kok = None
    for n in ["Kök Tip Kodu", "KökTip", "KokTip"]:
        if n in df_run.columns:
            col_kok = n
    ...
    out = df_run.copy()
    out["Tarak Grubu"] = (
        out[col_kok].astype(str).str.strip().str.upper().map(lambda x: tarak_map.get(x, ""))
    )
    out["_TarakKey"] = out["Tarak Grubu"].apply(_tarak_key)
    return out
